...
int main(void) {
    ...
    /* exercises pipe()/dup2()-backed pipeline wiring across two forked stages */
    if (wsh_send(&p, "echo hello wsh | grep wsh") != 0) {
        fprintf(stderr, "failed to send piped command\n");
        wsh_kill(&p);
        return 1;
    }
    if (wsh_read_until(&p, "» ", buf, sizeof(buf), 5000) < 0) {
        fprintf(stderr, "timed out waiting for piped output\n");
        wsh_kill(&p);
        return 1;
    }
    wsh_assert_contains(buf, "hello wsh", "piped output");
