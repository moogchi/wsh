...
int main(void) {
    ...
    if (wsh_send(&p, "cd /tmp") != 0) {
        fprintf(stderr, "failed to send cd command\n");
        wsh_kill(&p);
        return 1;
    }
    if (wsh_read_until(&p, "» ", buf, sizeof(buf), 5000) < 0) {
        fprintf(stderr, "timed out waiting for prompt after cd\n");
        wsh_kill(&p);
        return 1;
    }

    if (wsh_send(&p, "pwd") != 0) {
        fprintf(stderr, "failed to send pwd command\n");
        wsh_kill(&p);
        return 1;
    }
    if (wsh_read_until(&p, "» ", buf, sizeof(buf), 5000) < 0) {
        fprintf(stderr, "timed out waiting for pwd output\n");
        wsh_kill(&p);
        return 1;
    }
    wsh_assert_contains(buf, "/tmp", "pwd output");
