...
int main(void) {
    ...
    if (wsh_send(&p, "fds") != 0) {
        fprintf(stderr, "failed to send fds command\n");
        wsh_kill(&p);
        return 1;
    }
    if (wsh_read_until(&p, "» ", buf, sizeof(buf), 5000) < 0) {
        fprintf(stderr, "timed out waiting for fds output\n");
        wsh_kill(&p);
        return 1;
    }
    /* nothing has been opened yet, so the FD table should be empty */
    wsh_assert_contains(buf, "no tracked fds", "fds output");
